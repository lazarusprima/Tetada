...
export default function AdminStokPage() {
  ...
  const handleDelete = async () => {
    if (selectedSchedule) {
      ...
      if (!error) {
        window.dispatchEvent(new CustomEvent('app-notify', { detail: `telah menghapus sebuah Jadwal Distribusi Buah & Susu.` }));
        fetchSchedules();
        fetchStok();
      }
      closeDeleteModal();
  ...
  const updateStock = async (amount: number) => {
    ...
    if (!error) {
      if (newStock === 0) {
        window.dispatchEvent(new CustomEvent('app-notify', { detail: `telah membagikan stok terakhir. Stok Buah & Susu kini telah habis.` }));
      }
          ...
          <div className="flex flex-col sm:flex-row gap-[16px] w-full md:w-auto">
            <div className="relative flex-1 w-full sm:w-[320px]">
              ...
              <input
                ...
                className="w-full bg-[#F0F2F5] dark:bg-[#0a192f] border border-[#E2E8F0] dark:border-[#233554] rounded-[8px] pl-[44px] pr-[16px] py-[10px] text-[11.3px] text-[#A0AEC0] dark:text-white focus:outline-none focus:border-blue-500 transition-colors"
              />
            </div>
            <Link
